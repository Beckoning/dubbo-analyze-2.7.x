...
/**
 * DefaultFuture.
 * 可以把DefaultFuture看成是一个中介，买房和卖房都通过这个中介进行沟通，
 * 中介拥有着买房者的信息request和卖房者的信息response，并且促成他们之间的买卖
 *
 * 在该类中，把请求和响应通过唯一的id一一对应起来。
 * 做到异步处理返回结果时能给准确的返回给对应的请求。
...
public class DefaultFuture extends CompletableFuture<Object> {
    ...
    public static void received(Channel channel, Response response, boolean timeout) {
        try {
            ...
            if (future != null) {
                ...
                //timeout判断是否因为超时定时任务发起  不是超时定时任务发起，这是服务端调用（即请求的响应结果返回），可以关闭定时任务
                if (!timeout) {
                    // decrease Time 关闭定时任务 减少调用 提升效率
                    t.cancel();
                }
                // 接收响应结果
    ...
    /**
     * 该方法是取消一个请求，可以直接关闭一个请求，也就是值创建一个响应 来回应该请求，
     * 把response值设置到该请求对于到future中，做到了中断请求的作用。
     * 该方法跟closeChannel的区别是closeChannel中对response的状态设置了CHANNEL_INACTIVE，而cancel方法是中途被主动取消的，
     *
