...
public abstract class AbstractClusterInvoker<T> implements ClusterInvoker<T> {
     ...
     *
     * 该方法实现了使用负载均衡策略选择一个调用者。首先，使用loadbalance选择一个调用者。
    ...
    private Invoker<T> doSelect(LoadBalance loadbalance, Invocation invocation,
        ...
        //注解方案一
        //1、如果服务某个节点调用失败 且 这次筛选的还是调用之前调用失败的
        //2、服务可以获得 且url不为空 且这次选择的服务不可用   以上两种情况还需要再次筛选
    ...
    private Invoker<T> reselect(LoadBalance loadbalance, Invocation invocation,
        ...
        //如果筛选后服务不为空， 从筛选后的服务中选择一个执行（调用选择机制）
        if (!reselectInvokers.isEmpty()) {
            return loadbalance.select(reselectInvokers, getUrl(), invocation);
        }

        // Just pick an available invoker using loadbalance policy
        //如果筛选后服务为空，从调用失败的服务中选择可用的服务
        if (selected != null) {
    ...
    /**
     * 该方法是invoker接口必备的方法，调用链的逻辑，
     * 不过主要的逻辑在doInvoke方法中，该方法是该类的抽象方法，让子类只关注doInvoke方法。
     * @param invocation
     * @return
     * @throws RpcException
     */
